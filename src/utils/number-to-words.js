...
export function numberToWords(n) {
  ...
  if (n < 100) {
    ...
    const unit = n % 10;
    const sep = (unit === 1 && (ten === 1 || ten === 7 || ten === 9)) ? ' et ' : '-';
    if (ten === 7 || ten === 9) {
      // soixante-dix, quatre-vingt-dix
      const baseTen = TENS[ten];
      return unit
        ? `${baseTen}${sep}${TEENS[unit]}`
        : ten === 8
          ? `${baseTen}s`
          : baseTen;
    }
  ...
  if (n < 1000) {
    ...
    return rest
      ? `${hText} ${numberToWords(rest)}`
      : hText;
  }
  if (n < 1_000_000) {
    const thousands = Math.floor(n / 1000);
    const rest = n % 1000;
    const tText = thousands > 1
      ? `${numberToWords(thousands)} mille`
      : 'mille';
    return rest
      ? `${tText} ${numberToWords(rest)}`
      : tText;
  }
  if (n < 1_000_000_000) {
    const millions = Math.floor(n / 1_000_000);
    const rest = n % 1_000_000;
    const mText = millions > 1
      ? `${numberToWords(millions)} millions`
      : 'un million';
    return rest
      ? `${mText} ${numberToWords(rest)}`
      : mText;
  }
  return `${n}`;
}
...
export function amountToWords(amount, devise) {
  ...
  if (fraction) {
    words = `${words} et ${numberToWords(fraction)} centimes`;
  }

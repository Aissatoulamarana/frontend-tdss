...
import { PermissionTableToolbar } from '../permission-table-tollbar';
...
const TABLE_HEAD = [
    ...
    { id: 'profil', label: 'Type de Profil', width: 180 },
    { id: 'autorisation', label: 'Autorisations', width: 220 },
...
export function PermissionListView() {
    ...
    useEffect(() => {
        ...
        const fetchPermission = async () => {
            try {
                const response = await axios.get(API.listPermissions());
...
function applyFilter({ inputData, comparator, filters }) {
    ...
    if (name) {
        inputData = inputData?.filter(
            (permission) => permission?.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
        );
    }

    if (status !== 'all') {
        inputData = inputData?.filter((permission) => permission?.status === status);
    }

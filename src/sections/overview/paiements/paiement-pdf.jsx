...
export function PaiementPDF({ payment }) {
  ...
  const qrData = encodeURIComponent(
    `Paiement: ${payment?.number} - Facture: ${payment?.facture_number || ''} - Montant: ${payment?.amount || ''} ${payment?.devise?.sign || 'GNF'}`
  );
    ...
    <View style={styles.copyContainer}>
      ...
      <View style={[styles.column, { flex: 0.5, marginBottom: 1 }]}>
          ...
          >
            <Text
              style={[
                ...
                { width: 100, textAlign: 'right', marginRight: 5 },
              ]}
            >
              Facture N° :
            </Text>
        ...
        >
          ...
          <Text style={[styles.infoValue, styles.tableCellRight]}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.infoTableRow}>
            ...
            <Text style={styles.infoValue}>{payment?.client?.contact}</Text>
          </View>
          <View style={styles.infoTableRow}>
            <Text style={styles.infoLabel}>Adresse :</Text>
            <Text style={styles.infoValue}>{payment?.client?.adresse}</Text>
          </View>
        </View>
      ...
      <View style={styles.table}>
        <View style={styles.tableHeader}>
          <Text style={[styles.tableCell30, styles.infoLabel]}>Factures</Text>
          <Text style={[styles.tableCell40, styles.infoLabel, styles.tableCellCenter]}>
            Date de facturation
          </Text>
          <Text
            style={[
        ...
        {payment?.factures?.map((facture, idx) => (
          <View key={idx} style={styles.tableRow}>
            <Text style={styles.tableCell30}>{facture?.number} </Text>
            <Text style={[styles.tableCell40, styles.tableCellCenter]}>
              {' '}
              {formatDate(facture?.created_on)}{' '}
            </Text>
            <Text style={[styles.tableCell30, styles.tableCellRight, styles.tableCellLast]}>
              {convertirMontant(facture?.amount)}
            </Text>
          </View>
        ))}
        <View style={[styles.tableRow, { borderBottomWidth: 0 }]}>
          <Text style={styles.tableCell40} />
          <Text style={[styles.tableCell30, styles.infoLabel, styles.tableCellRight]}>
            TOTAL TTC
          </Text>
          <Text
            style={[
